...
- Dos modelos de datos ambientales, no intercambiables:
  - **Snapshot actual** (`GET /data/latest` → `DashboardSnapshot`, vía `getLatestSnapshot()`): semáforo, clima por estación (CGSM + Tasajera), satélite, calidad del agua, tendencias 24h/7d, señales de riesgo (anoxia/pulso de agua dulce/vendaval — outlooks, no alertas) y alertas de ciclón/tormenta (`senales.tormenta`, sistema convectivo real vía descargas GLM). Vive el momento presente, no series.
...
- `lib/api.ts` también define `getAlerts`/`AlertsResponse` (`GET /data/alerts`) sin usar en ninguna página todavía — las alertas de ciclón y tormenta que sí se muestran (header global) vienen del snapshot (`cyclone_alerts`/`senales.tormenta`), no de este endpoint.
